 ...
 *
 */
public class ConstraintGraphRW {
	...
	public Graph[] createGraph(Object[] NormOnt, String logBatch) throws Exception
	{
		...
		try{
			...
			for(OWLClass cls : ontology.getClassesInSignature()) 
			{
	            if(!cls.isOWLThing())
	            {
	            	gClass.insertNode(cls.toString(), NodeType.Class);
	            }
			}
			//Properties insertion
			for(OWLObjectProperty pro : ontology.getObjectPropertiesInSignature()) 
			{
	            gProperty.insertNode(pro.toString(), NodeType.Property);
			}
			...
			for(OWLSubClassOfAxiom subClass : ontology.getAxioms(AxiomType.SUBCLASS_OF)) 
			{
				if((!subClass.getSubClass().isOWLThing())&&(!subClass.getSuperClass().isOWLThing()))
				{
					...
					gClass = insertNodeSuperSubClassAxiom(superClassType,superClassObj,gClass);
					gClass = insertNodeSubSubClassAxiom(subClassType,subClassObj, gClass);
					gClass.insertEdge(subClassStr, superClassStr);
				}
			}
			...
			for(OWLSubObjectPropertyOfAxiom subProp : ontology.getAxioms(AxiomType.SUB_OBJECT_PROPERTY)) 
			{
				...
				gProperty.insertNode(SuperPropExp.toString(), NodeType.Property);
				gProperty.insertNode(SubPropExp.toString(), NodeType.Property);
				gProperty.insertEdge(SubPropExp.toString(), SuperPropExp.toString());
			}
			//Stage 1 End
			System.out.println("Classes and Properties");
			//Graph2Console(g);
			//Stage 2 Start
			HashMap<Integer, Node> verticesClass = gClass.getVertices();
	        List<LinkedList<Integer>> adjClass = gClass.getAdj();
	        
	        HashMap<Integer, Node> verticesProperty = gProperty.getVertices();
	        List<LinkedList<Integer>> adjProperty = gProperty.getAdj();
	        
			for(OWLSubObjectPropertyOfAxiom subProp : ontology.getAxioms(AxiomType.SUB_OBJECT_PROPERTY)) 
			{
				OWLObjectPropertyExpression p = subProp.getSubProperty();
	            OWLObjectPropertyExpression q = subProp.getSuperProperty();
	            for(int j = 0; j < adjProperty.size(); j++) 
				{
		            LinkedList<Integer> e = adjProperty.get(j);
	            	if(e.size() > 0 &&(e !=null) )
	            	{
	            		if(verticesProperty.get(e.get(0)).getNodeType().equals(NodeType.RestrictionCardinality)) 
	            		{
	            			NodeRestrictionCardinality nok = (NodeRestrictionCardinality) gProperty.getVertices().get(e.get(0));
            					...
            					(!q.getDomains(ontology).isEmpty()  &&  ! q.getRanges(ontology).isEmpty()))
	            			{
	                            ...
	                            if(nok.getDescProp().equals(UsefulOWL.returnPropObjectProperty(p)) && nok.getDescRangeOrDomain().equals(expDomainP)) 
	                            {
	                                ...
	                                gProperty.insertNode(l.toString(), NodeType.RestrictionCardinality);
	                                gProperty.insertEdge(nok.getExpression(), l.toString());
	                            }
